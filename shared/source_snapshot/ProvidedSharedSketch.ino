...
//      GPIO13 and GPIO35 are now UNALLOCATED. Do not reuse without a pin review.
...
//
// Control philosophy (hysteresis, reducers, balance gate, FAE, mist schedule,
// exhaust logic, mode semantics) is deliberately unchanged.
// ============================================================================
...
#define FW_DEVICE_FAMILY    "germination-chamber"
#define FW_BUILD_ID         __DATE__ " " __TIME__
#define DEFAULT_DEVICE_NAME "SHUNYA-dev-000"
...
// Anti-stagnation recirculation when otherwise idle
float recirc_period_min  = 10;    // minutes between cycles (0 disables)
float recirc_duration_sec = 90;   // seconds each cycle runs
...
// Sensor balance thresholds — when the two RH sensors diverge, mix first
// before allowing the humidifier to add more moisture.
...
#define PIN_RECIRC  19   // Internal recirculation fans (two fans in parallel on one relay)
...
// GPIO13 and GPIO35: previously reserved for a touch controller that does not
// exist. Now UNALLOCATED. Available for future allocation after a pin review.
// GPIO12 must stay free (MTDI strapping pin, selects flash voltage at boot).

enum DeviceMode { MODE_AUTO, MODE_ON, MODE_OFF };
enum DeviceKind { KIND_HUMIDIFIER, KIND_MISTING, KIND_RECIRC, KIND_EXHAUST };
...
Device devices[] = {
  ...
  { "Recirculation Fan",  "recirc",     PIN_RECIRC,  KIND_RECIRC,     MODE_AUTO, false },
...
bool          recircCycleRunning   = false;
unsigned long recircChangedAt      = 0;
...
bool          rhBalanceHold        = false; // true = recirc-only mixing gate, humidifier blocked
...
// Coldest temperature reducer retained for monitoring/readout.
...
//   4. RECIRC:    auto-on if humidifier OR exhaust on, sensors disagree,
//                 or periodic anti-stagnation cycle is due
//
void updateRelays() {
  unsigned long now = millis();
  ...
  int iRecirc  = findIdx(KIND_RECIRC);
  ...
  // ---- Recirc periodic-cycle state machine ----
  if (recirc_period_min > 0 && recirc_duration_sec > 0) {
    if (recircCycleRunning) {
      if (now - recircChangedAt >= (unsigned long)(recirc_duration_sec * 1000.0f)) {
        recircCycleRunning = false;
        recircChangedAt = now;
      }
    } else {
      if (now - recircChangedAt >= (unsigned long)(recirc_period_min * 60000.0f)) {
        recircCycleRunning = true;
        recircChangedAt = now;
      }
    }
  } else {
  ...
  // ---- Sensor balance gate (mix first, then humidify) ----
  ...
  bool autoMist = mistCycleRunning || mistManualRunning;

  // ---- RECIRCULATION FAN ----
  bool autoRecirc = autoHum || autoExhaust || rhBalanceHold || sensorsDisagree || recircCycleRunning;
  ...
  for (int i = 0; i < NUM_DEVICES; i++) {
    ...
    else if (i == iRecirc)  autoT = autoRecirc;
    else if (i == iExhaust) autoT = autoExhaust;

    bool target;
    if      (devices[i].mode == MODE_ON)  target = true;
...
void saveSettings() {
  ...
  prefs.putFloat("rp", recirc_period_min);
  prefs.putFloat("rd", recirc_duration_sec);
...
void loadSettings() {
  ...
  recirc_period_min   = prefs.getFloat("rp", recirc_period_min);
  recirc_duration_sec = prefs.getFloat("rd", recirc_duration_sec);
  ...
  for (int i = 0; i < NUM_DEVICES; i++) {
    ...
    if (devices[i].kind == KIND_MISTING && m == (uint8_t)MODE_ON) m = (uint8_t)MODE_AUTO;
...
void drawDisplay(bool fullRedraw = false) {
  ...
  else if (rhBalanceHold)                { st = "MIXING";      sc = C_ACCENT; }
...
<div class="hdr">
  ...
  <div class="hdr-stat">
    ...
    <span class="chip" id="cRC">RC</span>
    <span class="chip" id="cMX">mix</span>
...
<div class="adv">
  ...
  <div class="adv-grp">Circulation (recirc)</div>
  <div class="adv-r"><label>Run every (min)</label><input id="rp" type="number" step="1" min="0" max="60"></div>
  <div class="adv-r"><label>For (sec)</label><input id="rd" type="number" step="15" min="0" max="300"></div>
  ...
  <div class="note">Set period to 0 to disable a cycle. Recirc also runs automatically when the humidifier or exhaust is on, when RH sensors are outside the balance band, or when the two sensors disagree by &gt;1.5&deg;C. The misting output uses the old heater relay pin and should be kept short to avoid flooding trays.</div>
</div></details>
  ...
  return order.map(i => {
    ...
    return `<div class="dev" data-id="${i}">
      ...
      <div class="pill" data-m="${mk}" data-i="${i}">
        ...
        <button data-k="on"${mk === 'on' ? ' class="act"' : ''}>${midLabel}</button>
...
['hb','fp','fd','rp','rd','mp','md','rbb','pcd'].forEach(id => {
...
async function refresh(){
  ...
  try {
    ...
    chip('cRC', byKind.recirc);     chip('cEX', byKind.exhaust);
    ...
    setIfBlur('rp', d.recirc_period);
    setIfBlur('rd', d.recirc_duration);
...
void handleStatus() {
  ...
  static char buf[3800]; int n = 0;   // static: keep it off the loop task stack
  ...
  n += snprintf(buf+n, sizeof(buf)-n,
    ...
    "\"ota\":{\"busy\":%s,\"configured\":%s,\"interval_min\":%lu,\"checks\":%lu,"
  ...
  n += snprintf(buf+n, sizeof(buf)-n,
    ...
    "\"fae_period\":%.0f,\"fae_duration\":%.0f,\"recirc_period\":%.0f,\"recirc_duration\":%.0f,"
    ...
    "\"fae_active\":%s,\"recirc_cycle\":%s,\"sensors_disagree\":%s,"
    ...
    fae_period_min, fae_duration_sec, recirc_period_min, recirc_duration_sec,
    ...
    faeRunning ? "true" : "false", recircCycleRunning ? "true" : "false", sensorsDisagree ? "true" : "false",
...
void handleConfig() {
  ...
  if (server.hasArg("rp"))  { recirc_period_min   = clampf(server.arg("rp").toFloat(), 0,   60);  changed = true; }
  if (server.hasArg("rd"))  { recirc_duration_sec = clampf(server.arg("rd").toFloat(), 0,   300); changed = true; }
...
void handleCheckUpdate() {
  otaRequestCheck(true);
  server.send(200, "application/json", "{\"ok\":true,\"msg\":\"check requested, poll /api/status ota.result\"}");
}
...
void loop() {
  ...
  if (wifiState == WS_CONNECTED && otaNextCheckAt && !otaBusy && otaManifestUrl.length() &&
