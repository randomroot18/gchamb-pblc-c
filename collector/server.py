...
HTML = b'''<!doctype html><html><meta charset="utf-8"><title>Jen chamber</title>
...
<h1>Jen chamber</h1><p id="state">Loading...</p><table><thead><tr><th>Received UTC</th><th>S1 C</th><th>S2 C</th><th>RH 1/2</th><th>Heater</th><th>Hum/Mist/Exh</th></tr></thead><tbody id="rows"></tbody></table>
...
def serve(db_path, device_token, admin_password, host, port):
    ...
    class Handler(BaseHTTPRequestHandler):
        def send_bytes(self, code, data, ctype='application/json'):
            ...
            if code == 401: self.send_header('WWW-Authenticate', 'Basic realm="Jen chamber"')
            self.end_headers()
        ...
        def do_POST(self):
            ...
            try:
                ...
                if obj['family'] != 'germination-chamber-jen-v1': raise ValueError('family')
...
if __name__ == '__main__':
    ...
    ap.add_argument('--db', default='jen.sqlite3'); ap.add_argument('--host',default='127.0.0.1')
    ap.add_argument('--port',type=int,default=8765)
    args = ap.parse_args()
    device_token = os.environ.get('JEN_DEVICE_TOKEN','')
    admin_password = os.environ.get('JEN_ADMIN_PASSWORD','')
    if len(device_token) < 32 or len(admin_password) < 16:
        ap.error('Set JEN_DEVICE_TOKEN (32+ chars) and JEN_ADMIN_PASSWORD (16+ chars) in environment')
